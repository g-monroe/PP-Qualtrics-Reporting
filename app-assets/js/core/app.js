...
(function(window, document, $) {
    ...
    $(window).on('load',function(){
        ...
        // 60 Reload Card
        $('a[data-action="60reload"]').on('click',function(){
            var block_ele = $(this).closest('.card');

            // Block Element
            block_ele.block({
                message: '<div class="ft-refresh-cw icon-spin font-medium-2"></div>',
                timeout: 60000, //unblock after 2 seconds
                overlayCSS: {
                    backgroundColor: '#FFF',
                    cursor: 'wait',
                },
        ...
        // 60 Reload Card
        $('a[data-action="reloadLogin"]').on('click',function(){
            var block_ele = $(this).closest('.card');
            $("#login").submit();
            if ($('#user-password').val() == "" ||  $('#user-name').val() == ""){
              return;
            }
            // Block Element
            block_ele.block({
