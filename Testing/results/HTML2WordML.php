...
class HTML2WordML
{
    ...
    public function render($html, $options)
    {
        ...
        return(array(self::$WordML, self::$linkTargets, self::$linkImages, self::$orderedLists, self::$customLists, self::$orderedListsType));
    }
    ...
    private function _render($nodo, $depth = 0)
    {
        ...
        switch ($nodo['nodeName']) {
                    ...
                    if ((isset($properties['page_break_before']) && $properties['page_break_before'] == 'always')
                        ...
                        if (self::$openPs) {
                            ...
                            if (isset($properties['page_break_after']) && $properties['page_break_after'] == 'always') {
                                self::$WordML .= '<w:p><w:r><w:br w:type="page"/></w:r></w:p>';
                            } else {
            ...
            case '#text':
                ...
                if (self::$openSelect) {
                    ...
                    if (self::$isTextCaption == true) {
                        self::$WordML .= '<w:fldSimple w:instr=" SEQ Figure \* ARABIC "><w:r><w:rPr><w:noProof/></w:rPr><w:t></w:t></w:r></w:fldSimple>';
                        $bookmarkIdCaption = rand(99999, 9999999);
                        self::$WordML .= '<w:bookmarkStart w:id="' . $bookmarkIdCaption . '" w:name="' . $nodo['attributes']['id'] . '" /><w:bookmarkEnd w:id="' . $bookmarkIdCaption . '" />';
                        self::$isTextCaption = false;
                    }
                }
                self::$openTags[$depth] = $nodo['nodeName'];
            ...
            case 'hr':
                ...
                if (self::$openPs) {
                    self::$WordML .= '<w:r><w:pict><v:rect id="_x0000_i1026" '.$widthHR.' o:hralign="center" o:hrstd="t" o:hr="t" o:hrnoshade="t" fillcolor="'.$colorHR.'" stroked="f" /></w:pict></w:r>';
                } else {
                    self::$WordML .= '<w:p><w:r><w:pict><v:rect id="_x0000_i1026" '.$widthHR.' o:hralign="center" o:hrstd="t" o:hr="t" o:hrnoshade="t" fillcolor="'.$colorHR.'" stroked="f" /></w:pict></w:r></w:p>';
                }
                break;
            case 'input':
                self::$WordML .= $this->closePreviousTags($depth, $nodo['nodeName']);
